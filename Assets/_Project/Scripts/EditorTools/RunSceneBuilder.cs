...
namespace ChibiRift.EditorTools
{
    ...
    public static class RunSceneBuilder
    {
        ...
        /// <summary>One Ground tileset cell in world units: 16px at 32 PPU (P2 slice 1, A1).</summary>
        private const float TileWorldSize = 0.5f;

        private const string GroundTilePath = "Assets/_Project/Art/Tileset/Ground/Tile_Placeholder.asset";

        /// <summary>
        /// Ground split around a 3u hole at x = -8, plus one platform — all Tilemap now (P2 slice 1,
        /// A5) — and two walls, still simple boxes. Every world-space number is unchanged from P1
        /// slice 1, only the ground/platform collision mechanism moved from BoxCollider2D to
        /// Tilemap + TilemapCollider2D + CompositeCollider2D, so every existing position-dependent
        /// test (the hole, the wall stop, the platform landing) keeps meaning exactly what it did.
        /// </summary>
        private static void BuildGeometry()
        {
            ...
            Tilemap groundTilemap = BuildGroundTilemap(ground);

            // The hole is cut by painting the floor as two slabs rather than one.
            float holeLeft = HoleCentreX - HoleWidth / 2f;
            float holeRight = HoleCentreX + HoleWidth / 2f;

            float leftWidth = holeLeft - (-ArenaHalfWidth);
            PaintGroundRect(groundTilemap,
                new Vector2(-ArenaHalfWidth + leftWidth / 2f, GroundY), new Vector2(leftWidth, 1f));

            float rightWidth = ArenaHalfWidth - holeRight;
            PaintGroundRect(groundTilemap,
                new Vector2(holeRight + rightWidth / 2f, GroundY), new Vector2(rightWidth, 1f));

            PaintGroundRect(groundTilemap, new Vector2(6f, 3f), new Vector2(6f, 0.5f)); // Platform
        ...
        /// <summary>
        /// One <c>Grid</c> + <c>Tilemap</c>, collision generated by <c>CompositeCollider2D</c>
        /// merging every painted tile into as few colliders as possible — the Tilemap equivalent of
        /// the old two-box-plus-platform layout, ready for the real Ground tileset to paint over
        /// once it exists. <c>CompositeCollider2D</c> needs a <c>Rigidbody2D</c> on the same object
        /// to generate anything; Static, since the ground never moves.
        /// </summary>
        private static Tilemap BuildGroundTilemap(int groundLayer)
        {
            var gridObject = new GameObject("Ground") { layer = groundLayer };
            var grid = gridObject.AddComponent<Grid>();
            grid.cellSize = new Vector3(TileWorldSize, TileWorldSize, 0f);

            var tilemapObject = new GameObject("Tilemap") { layer = groundLayer };
        ...
        /// <summary>Fills the world-space rectangle at <paramref name="centre"/>/<paramref name="size"/> with the ground tile.</summary>
        private static void PaintGroundRect(Tilemap tilemap, Vector2 centre, Vector2 size)
        {
            TileBase tile = LoadOrCreateGroundTile();
            Grid grid = tilemap.layoutGrid;

            int columns = Mathf.RoundToInt(size.x / TileWorldSize);
            int rows = Mathf.RoundToInt(size.y / TileWorldSize);

            var origin = new Vector3(centre.x - size.x / 2f, centre.y - size.y / 2f, 0f);
            ...
            for (int x = 0; x < columns; x++)
            {
                ...
                {
                    tilemap.SetTile(new Vector3Int(originCell.x + x, originCell.y + y, 0), tile);
                }
            }
        }

        /// <summary>
        /// The one placeholder tile every painted ground cell uses until the real Ground tileset
        /// exists — same principle as <see cref="AddPlaceholderSprite"/>: a real asset on disk, not
        /// an in-memory object a scene cannot reliably keep a reference to.
        /// </summary>
        /// <remarks>
        /// Deletes and recreates the asset every run rather than loading and keeping whatever is
        /// already there (OI-35). An earlier version returned the existing asset early, which meant
        /// the very first fix attempted here — setting <c>colliderType</c> to <c>Grid</c> — silently
        /// never reached the asset already on disk from a prior run: the tile kept its original
        /// <c>Sprite</c> collider type, whose shape comes from a physics outline this placeholder
        /// texture never generates, so it contributed no collision at all. The bug looked like a
        /// Tilemap/Composite wiring problem; it was a caching problem one layer up.
        /// </remarks>
        private static TileBase LoadOrCreateGroundTile()
        {
            AssetDatabase.DeleteAsset(GroundTilePath);
            Directory.CreateDirectory(Path.GetDirectoryName(GroundTilePath));

            var tile = ScriptableObject.CreateInstance<Tile>();
            tile.sprite = PlaceholderArt.LoadOrCreateTileSprite();
            tile.color = new Color(0.30f, 0.34f, 0.40f); // matches the old Ground_Left/Right colour

            // Grid gives a full solid cell regardless of the placeholder sprite's own outline.
            // Tile.colliderType's default (Sprite) derives its shape from the sprite's physics
            // outline, which this placeholder texture never generates — TilemapCollider2D then
            // silently treats the tile as "nothing here" rather than as an error (see the remarks
            // above on why leaving this unset went unnoticed for a whole run).
